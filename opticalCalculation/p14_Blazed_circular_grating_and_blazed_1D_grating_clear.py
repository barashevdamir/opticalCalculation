...
def axicon_and_1d_grating(N=500, Pr=20, P=10):
    ...
    fig1 = go.Figure(data=[go.Surface(z=r)])
    fig1.update_layout(title='', autosize=False, width=600, height=600)
    fig1.show()

    P1 = np.remainder(r, Pr)  # Construction of blazed axicon
    A1 = (P1 / Pr) * 2 * np.pi

    fig2 = go.Figure(data=[go.Surface(z=A1)])
    fig2.update_layout(title='', autosize=False, width=600, height=600)
    fig2.show()

    P2 = np.remainder(X, P)  # Construction of blazed 1D grating on X
    A2 = (P2 / P) * 2 * np.pi

    fig3 = go.Figure(data=[go.Surface(z=A2)])
    fig3.update_layout(title='', autosize=False, width=600, height=600)
    fig3.show()

    P3 = np.remainder(Y, P)  # Construction of blazed 1D grating on Y
    A3 = (P3 / P) * 2 * np.pi

    A = np.remainder(A2 + A3, 2 * np.pi)  # Modulo-2pi phase addition of the two phase profiles

    fig4 = go.Figure(data=[go.Surface(z=A)])
    fig4.update_layout(title='', autosize=False, width=600, height=600)
    fig4.show()

    fig5 = go.Figure()
    fig5.add_trace(go.Heatmap(z=A, colorscale='gray'))
